export default async function handleKVSync(request: Request, env: Env) {
  ...
  try {
    ...
    await env.KV_INCROFT_JWT_AUTH.put(kv_key, kv_value);

    // ✅ Fetch it back and ensure it's retrievable
    const storedValue = await env.KV_INCROFT_JWT_AUTH.get(kv_key);
    if (storedValue === null) {
